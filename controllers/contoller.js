    ...
    exports.postSignIn = (req, res, next) => {
     const {email , password}  = req.body;
      ...
      User.findOne({email}).then(userInfo =>{
        ...
        bcrypt.compare(userInfo.password , password).then(data =>{
          res.json({
    ...
    exports.postReset = (req, res, next) => {
      crypto.randomBytes(32, (err, buffer) => {
        ...
        if (err) {
          console.log(err);
          return res.json({"message" : "something went wrong ! check your script and try again"});
        }
        ...
        User.findOne({email})
          ...
          .then(result => {
            ...
            transporter.sendMail({
              to: req.body.email,
              from: 'kentinhogbnouto1@gmail.com',
              subject: 'Password reset',
              html: `
                <p>Click this <a href="http://localhost:8080/user-reset-pass/${token}">link</a> to set a new password.</p>
              `
            });
          })
          .catch(err => {
